...
namespace WindowsFormsApp1
{
    class Tritemius : str
    {
        ...
        public string encryption(string b, bool dec)
        {
            ...
            while (i != b.Length)
            {
                if (st_letters.Contains(b[i]))
                {
                    ...
                    if (dec) key = -1 * key % st_letters.Length;
        ...
        public string encryption_motto(string b, bool dec)
        {
            ...
            while (i != b.Length)
            {
                if (st_letters.Contains(b[i]) && st_letters.Contains(st_key[n]))
                {
                    int index = st_letters.IndexOf(b[i]);
                    int index_key = st_letters.IndexOf(st_key[n]);

                    if (dec) index_key *= -1;

                    int y = (index + index_key + st_letters.Length) % st_letters.Length;
                    a += st_letters[y];
                    n++;
                }
                else
