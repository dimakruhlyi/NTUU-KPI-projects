...
<body>
	...
	if (isset($_POST['id_change_item'])) {
		$id_change_item = $_POST['id_change_item'];
		if ($id_change_item == '') {
			unset($id_change_item);
		}
	}
	...
	$query = mysql_query("UPDATE `item` SET `name_item`='$name_item',`id_category`='$id_item_category',`price_item`='$price_item',`count_item`='$count_item',`sold_item`='$sold_item' WHERE `id_item` = '$id_change_item'", $bd);
	if ($query == 'true') {
